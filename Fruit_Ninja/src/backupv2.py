...
def fruit_spawn_gm2():
    ...
    if bomb_chance != 4:
        ...
        fruit_type = scale_image(fruit_type,75,100)
        isbomb = True
    place1 = random.choice(list(starting_pos.keys()))
    ...
    vel_x = distance_x/(distance*1.2) *20
    vel_y = distance_y/(distance*1.2) *20
...
while run:
    ...
    if running_state == "playing" and gamemode == "normal":
        ...
        for i, item in enumerate(menu_options):
            ...
            text_center = center(text,i*75+100)
        ...
        for fruit in fruits_gm2[:]: #shallow copy of list
            ...
            if (fruit[0]>Screen_width or fruit[0]<0) and (fruit[1]>Screen_length or fruit[1]<0) and fruit in fruits:
                fruits.remove(fruit)
            if hold_mouse and previous_mouse:
                if fruit_hitbox.clipline(previous_mouse,mouse_position):
                    if not fruit[5]:
        ...
        if running_state == "playing" and gamemode == "normal":
            ...
            if event.type == pg.KEYDOWN:
                if event.key == pg.K_DOWN:
                    ...
                    run = False
        elif running_state == "playing" and gamemode == "ALERT":
            if event.type == pg.MOUSEBUTTONDOWN:
                hold_mouse = True
            if event.type == pg.MOUSEBUTTONUP:
                hold_mouse = False
